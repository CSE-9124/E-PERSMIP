          ...
          <nav className="hidden lg:flex items-center space-x-2">
            {navLinks.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className={`px-4 py-2 rounded-lg font-semibold transition-all duration-150 text-white/90 hover:bg-gray-200 hover:bg-opacity-20 hover:text-gray-200 ${
            ...
            <div className="bg-white/10 rounded-2xl p-4 space-y-2">
              ...
              <div className="border-t border-white/20 pt-3 mt-3">
                <div className="flex items-center justify-between">
                  <span className="text-white font-semibold text-sm bg-red-900 bg-opacity-30 px-3 py-1 rounded-xl">Admin</span>
                  <button
                    onClick={() => {
                      localStorage.removeItem('isLoggedIn')
                      window.location.href = '/login'
